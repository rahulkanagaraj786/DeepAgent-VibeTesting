...
The entire server.py is generated by Gemini in a single pass.
No scaffolding, no stitching — pure generation.
Validated with ast.parse() and auto-repaired if needed.
Generated servers use mcp.server.fastmcp.FastMCP for Blaxel hosting.

Provider: Google Gemini (GEMINI_API_KEY → gemini-2.5-flash)
"""
...
def generate(
    ...
    with log_stage("Agentic Code Generation"):
        ...
        logger.info("Provider: Gemini (gemini-2.5-flash)")
